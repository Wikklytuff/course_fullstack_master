window.addEventListener('load', function() {
    ...
    var leftArrow = this.document.querySelector('.lf'); // 取lf类的第一个元素
    var rightArrow = this.document.querySelector('.lr');
 
    // 鼠标移动到左右箭头的位置更换图片 有灰色背景的图片
    // 雪碧图的使用
    // 这里用 addEventListener 因为传统的onclick如果 item.onclick = function() { alert('What'); } 
    // 和 item.onclick = function() { alert('Yo.'); } 只会输出“Yo.",前面的"What"会被覆盖。
    ...
    for (let k = 0; k < dots.length; k++) {
        // 添加自定义属性data-index判断与哪一张图片对应切换效果
